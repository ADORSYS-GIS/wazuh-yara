...
install_yara_from_local_archive() {
    ...
    # Resolve archive path relative to this script (../archives)
    local script_dir
    ...
    if ! tar -xzf "$archive_path" -C "$temp_extract"; then
        error_message "Failed to extract local YARA archive"
        return 1
    fi
    ...
    case "$OS" in
        linux)
            ...
            if [ "${DISTRO:-}" = "ubuntu" ]; then
                ...
                if [ -z "$ubuntu_version" ] && [ -f /etc/os-release ]; then
                    # shellcheck source=/dev/null
                    . /etc/os-release
...
install_yara_redhat() {
    ...
    if ! yum repolist | grep -q epel; then
        ...
        if command_exists dnf; then
            maybe_sudo dnf install -y epel-release || true
        else
            maybe_sudo yum install -y epel-release || true
        fi
    fi
    print_step 2 "Installing YARA package..."
    if command_exists dnf; then
        maybe_sudo dnf install -y yara
    else
        maybe_sudo yum install -y yara
    fi
    success_message "YARA installed successfully using yum/dnf."
    print_step 3 "Downloading YARA rules..."
    download_yara_rules
    print_step 4 "Downloading yara.sh script..."
    download_yara_script
        ...
        linux)
            case "$DISTRO" in
                ubuntu|debian)
                    ...
                    ;;
                centos|rhel|redhat)
                    info_message "Using yum installation for RedHat-based Linux ($DISTRO)"
                    install_yara_redhat
                    ;;
                *)
                    error_message "Unsupported Linux distribution: $DISTRO"
                    error_message "This script only supports Ubuntu, Debian, and RedHat-based distributions"
                    exit 1
                    ;;
            esac
            ;;
